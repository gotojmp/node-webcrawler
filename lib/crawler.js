...
Crawler.prototype.init = function init (options) {
    ...
    var defaultOptions = {
        ...
        rotateUA:               false,
        download:               false //filepath or false
    };
...
Crawler.prototype._pushToQueue = function _pushToQueue (options) {
    ...
    var acquired = function (error, poolReference) {
        ...
        if (self.debug) {
            ...
            logger.info("pool queue size:%s, waiting size:%s, bottleneck '%s' queue size:%s", self.queueSize, self.waitingCount, options.limiter||"default", self.limiters.key(options.limiter||"default")._queue.length);
        }
...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    if (ropts.forceUTF8 && ropts.json) { // when forced utf8 json maybe decoded error
        ropts.json = false;
    }
